...
class Game {
    ...
    movePawn(destination){
        ...
        const jumpOverPawn = (currentPos, destPos) => {
            ...
            if(hopsOffBoard(currentPos[0], currentPos[1], direction)){
                return {success: false, message: `not enough space in direction of travel`};
            }
            switch (direction){
                case 'up':
                    ...
                    if(checkForWall(currentPos[0] + 3, currentPos[1])){
                        return {success: false, message: `Wall on opposite side of enemy pawn`};
                    }
    ...
    placeWall(row, col){  
        ...
        if (this.board[row][col].occupiedBy !== null){
           return {success: false, message: `row: ${row} and col: ${col} are occupied`};
        }
        ...
        if (orientation === false){
            return {success: false, message: `Cannot place wall in a space or intersection`};
        } 
    }
}
module.exports = Game;